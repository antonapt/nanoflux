...
if [[ $# -ne 3 ]]; then
	echo "Usage: $0 <sample_id> <quantile> <metric>"
	echo "Example: $0 N2024-4049 0.1 nontumor_score_mean"
	exit 1
fi
...
setting_id="metric_${metric}_quantile_${quantile}_with_intervals"
...
echo "[INFO] Running approach 3 (bin-based) for sample=$sample_id metric=$metric quantile=$quantile"
