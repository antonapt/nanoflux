...
all_min_cpg_filters=( 2 3 4 5 10 20 50)


SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
RUNNER="$SCRIPT_DIR/run_prepare_n_infer_one_setting_with_only_short_read_filtering.sh"
...
export LOG_DIR STATUS_DIR RUNNER
echo "[INFO] Writing per-job logs to: $LOG_DIR"

job_count=$(( ${#all_samples[@]} * ${#all_min_cpg_filters[@]} ))

for sample_id in "${all_samples[@]}"; do
    for min_cpg_filter in "${all_min_cpg_filters[@]}"; do
        printf '%s\t%s\n' "$sample_id" "$min_cpg_filter"
    done
done |
xargs -P "$MAX_JOBS" -n 2 bash -c '
    set -uo pipefail
    sample_id="$1"
    min_cpg_filter="$2"
    job_key="${sample_id}__short_read_min_cpgs_${min_cpg_filter}"
    log_file="$LOG_DIR/${sample_id}/${job_key}.log"
    status_file="$STATUS_DIR/${job_key}.status"
    mkdir -p "$LOG_DIR/${sample_id}"

    set +e
    echo "[START] sample=$sample_id min_cpg=$min_cpg_filter" >"$log_file"
    "$RUNNER" "$sample_id" "$min_cpg_filter" >>"$log_file" 2>&1
    exit_code=$?

    if [[ "$exit_code" -eq 0 ]]; then
        echo "[DONE]  sample=$sample_id min_cpg=$min_cpg_filter" >> "$log_file"
    fi

    if [[ "$exit_code" -eq 0 ]]; then
        echo "ok" > "$status_file"
    else
        echo "[FAILED] sample=$sample_id min_cpg=$min_cpg_filter exit_code=$exit_code" >> "$log_file"
