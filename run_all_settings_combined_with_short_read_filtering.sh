...
all_min_cpg_filters=( 1 5 10 20 )
all_short_read_filters=( 2 5 10 20 )
all_metrics=('nontumor_score_mean' 'nontumor_score_sum')


SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
RUNNER="$SCRIPT_DIR/run_prepare_n_infer_one_setting_combined_with_short_read_filtering.sh"
...
job_count=$(( ${#all_samples[@]} * ${#all_quantiles[@]} * ${#all_min_cpg_filters[@]} * ${#all_short_read_filters[@]} * ${#all_metrics[@]} ))
        ...
        for min_cpg_filter in "${all_min_cpg_filters[@]}"; do
            for short_read_filter in "${all_short_read_filters[@]}"; do
                for metric in "${all_metrics[@]}"; do
                    printf '%s\t%s\t%s\t%s\t%s\n' "$sample_id" "$quantile" "$min_cpg_filter" "$short_read_filter" "$metric"
                done
            done
        done
    done
done |
xargs -P "$MAX_JOBS" -n 5 bash -c '
    set -uo pipefail
    sample_id="$1"
    quantile="$2"
    min_cpg_filter="$3"
    short_read_filter="$4"
    metric="$5"
    job_key="${sample_id}__q${quantile}__cpg${min_cpg_filter}__srf${short_read_filter}__${metric}"
    log_file="$LOG_DIR/${sample_id}/${job_key}.log"
    status_file="$STATUS_DIR/${job_key}.status"
    mkdir -p "$LOG_DIR/${sample_id}"

    set +e
    echo "[START] sample=$sample_id quantile=$quantile min_cpg=$min_cpg_filter short_read_filter=$short_read_filter metric=$metric" >"$log_file"
    "$0" "$sample_id" "$quantile" "$min_cpg_filter" "$short_read_filter" "$metric" >>"$log_file" 2>&1
    exit_code=$?

    if [[ "$exit_code" -eq 0 ]]; then
        echo "[DONE]  sample=$sample_id quantile=$quantile min_cpg=$min_cpg_filter short_read_filter=$short_read_filter metric=$metric" >> "$log_file"
    fi

    if [[ "$exit_code" -eq 0 ]]; then
        echo "ok" > "$status_file"
    else
        echo "[FAILED] sample=$sample_id quantile=$quantile min_cpg=$min_cpg_filter short_read_filter=$short_read_filter metric=$metric exit_code=$exit_code" >> "$log_file"
